...
def train(train_loader, model, criterion, optimizer, epoch, log, Ddim):
  ...
  for i, (input, target) in enumerate(train_loader):
    ...
    eye_var = torch.autograd.Variable(eye)

    # compute output
    output = model(input_var, eye_var)
...
def validate(val_loader, model, criterion, log, Ddim):
  losses = AverageMeter()
  top1 = AverageMeter()
  top5 = AverageMeter()
